...
def elmo_vectors_elmo(sess,embeddings):
...
def elmo_vectors_default(sess,embeddings):
...
def read_txt(path):
    ...
    with open(path,'r') as f:
        lns = f.readlines()
        for x in lns:
            res.append([float(y) for y in x[:-1].split()])
    return res

# def create_elmo_rep(dataset,data_version,types=['default']): #types=['default','elmo]
#     conf = {
#         'bachsize':10,
...
#     tempsavepath = conf["savepath"]+"_tempsavepath.pkl"
#     f = open(tempsavepath,"w")
#     data , ids = prepare_data(conf["datapath"],conf["idspath"])
#     # data = data[:2]
...
#         # elif types[0] == 'default':
#         #     def_emb.extend([x.tolist() for x in elmo_vectors_default(embeddings["default"])])
#         # elif types[0] == 'elmo':
#         #     elmo_emb.extend([x.tolist() for x in elmo_vectors_elmo(embeddings["elmo"])])
#
#         if len(types)>1:
#             f.write(" ".join([x.tolist() for x in elmo_vectors_elmo(embeddings["elmo"])]))
#             f1.write(" ".join([x.tolist() for x in elmo_vectors_default(embeddings["default"])]))
#             f.write("\n")
#             f1.write("\n")
#         elif types[0] == 'default':
#             for x in elmo_vectors_elmo(embeddings["elmo"]):
#                 f.write(" ".join([str(y) for y in x.tolist()]))
#                 f.write("\n")
#         elif types[0] == 'elmo':
#             f1.write(" ".join([x.tolist() for x in elmo_vectors_default(embeddings["default"])]))
#
...
#
#     if len(types)>1:
#         elmo_emb = read_txt(tempsavepath)
#         def_emb = read_txt(tempsavepath)
#         save_data(conf["savepath"]+"_elmo-weighted.pkl",[elmo_emb,ids])
#         save_data(conf["savepath"]+"_elmo-default.pkl",[def_emb,ids])
#     elif types[0] == 'default':
#         def_emb = read_txt(tempsavepath)
...
def create_elmo_rep(dataset,data_version,types=['default']): #types=['default','elmo]
    ...
    for s in range(steps):
        ...
        if len(types)>1:
            elmo_emb.extend([x.tolist() for x in elmo_vectors_elmo(sess,embeddings["elmo"])])
            def_emb.extend([x.tolist() for x in elmo_vectors_default(sess,embeddings["default"])])
        elif types[0] == 'default':
            def_emb.extend([x.tolist() for x in elmo_vectors_default(sess,embeddings["default"])])
        elif types[0] == 'elmo':
            elmo_emb.extend([x.tolist() for x in elmo_vectors_elmo(sess,embeddings["elmo"])])
